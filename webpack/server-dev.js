...
module.exports = {
  ...
  target: 'node',
  watch: true,
  entry: [webpackHotEntry, './server/index.tsx'],
  output: {
  ...
  module: {
    rules: [
        ...
        oneOf: [
          {
            ...
            options: {
              modules: { mode: 'global', localIdentName: rules.localIdentName },
              onlyLocals: true,
            },
          },
          {
            loader: 'css-loader',
            options: {
              modules: { mode: 'local', localIdentName: rules.localIdentName },
              onlyLocals: true,
            },
          },
        ],
      },
      {
        test: rules.imgTest,
        loader: 'url-loader',
        options: { limit: 10000, emitFile: false, name: rules.devFileName },
      },
      {
        test: rules.fontTest,
        loader: 'url-loader',
  ...
  plugins: [
    ...
    new WebpackShellPlugin({
      // 'webpack watch: true' runs indefinitely, so to be able to start the server right after the initial build, we need to use this plugin
      onBuildEnd: {
        scripts: [
          `node ${path.join(__dirname, 'waitUntilLoadableStatsCreated')} && node ${path.join(
            serverDevOutputPath,
            'server.js'
          )}`,
        ],
